[Mesh]
  ...
  []
  [create_cylinder_jet_top_sideset]
    input = create_fused_right_sideset
    type = ParsedGenerateSideset
    included_boundaries = 'circle'
    combinatorial_geometry = 'y > 0 & abs(x) < ${jet_x_extent}'
    new_sideset_name = 'cylinder_jet_top'
  []
  [create_cylinder_jet_bottom_sideset]
    input = create_cylinder_jet_top_sideset
    type = ParsedGenerateSideset
    included_boundaries = 'circle'
    combinatorial_geometry = 'y < 0 & abs(x) < ${jet_x_extent}'
    new_sideset_name = 'cylinder_jet_bottom'
  []
  [create_circle_solid_sideset]
    input = create_cylinder_jet_bottom_sideset
    type = ParsedGenerateSideset
    included_boundaries = 'circle'
    excluded_boundaries = 'cylinder_jet_top cylinder_jet_bottom'
    combinatorial_geometry = '1'
    new_sideset_name = 'circle_solid'
  []
  [sideset_removal]
    input = create_circle_solid_sideset
    type = BoundaryDeletionGenerator
    boundary_names = 'circle bottom top left right middle_bottom middle_top bottom_top top_bottom'
  []
[]
